...
const SelectFont = ({font, handleFontSelect, otherFonts, handleActiveSelect, comparedFont, selectIsActive}) => {
  return (
    <div className={'w-9 flex flex-col text-white max-3xl: absolute h-11 top-0 right-[120px]'}>
      <div
        onClick={handleActiveSelect}
        className={`${font.className} text-sm flex bg-black/80 p-1 cursor-pointer rounded border-solid border`}>{comparedFont.title}
